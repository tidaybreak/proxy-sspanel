...
class User(AbstractUser):
    ...
    out_tag = models.CharField("出口tag", max_length=255, default="", null=True)
    ...
    def update_from_dict(self, data):
        clean_fields = ["proxy_password", "ss_password", "ss_method"]
...
class UserHttpConfig(models.Model, UserPropertyMixin):
    # TODO delete this table
    MIN_PORT = 1025
    PORT_BLACK_SET = {6443, 8472}
...
class HttpNode(BaseAbstractNode):
    KB = 1024
    MEGABIT = KB * 125

    server = models.CharField("服务器地址", max_length=128)
    port = models.IntegerField("端口", default=31300)
    speed_limit = models.IntegerField("限速", default=0)

    class Meta:
        verbose_name_plural = "Http节点"

    @classmethod
    @cache.cached(ttl=60 * 60 * 24)
    def get_user_http_configs_by_node_id(cls, node_id):
        http_node = cls.get_or_none_by_node_id(node_id)
        configs = {"users": []}
        if not http_node:
            return configs
    ...
    def api_endpoint(self):
        params = {"token": settings.TOKEN}
        return (
                settings.HOST + f"/api/user_http_config/{self.node_id}/?{urlencode(params)}"
        )
    ...
    def to_dict_with_extra_info(self, user):
        ...
        data.update(
            NodeOnlineLog.get_latest_online_log_info(
                NodeOnlineLog.NODE_TYPE_HTTP, self.node_id
            )
        )
        # if self.custom_method:
        #     data["method"] = user.ss_method
        ...
        data["country"] = self.country.lower()
        data["http_link"] = self.get_http_link(user)
        data["api_point"] = self.api_endpoint
        data["human_speed_limit"] = self.human_speed_limit
        return data
